...
if __name__ == '__main__': 
    ...
    train_dataset = DETRData(r"C:\Users\paulp\Downloads\Train") 
    train_dataloader = DataLoader(train_dataset, batch_size=4, collate_fn=stacker, drop_last=True) 

    test_dataset = DETRData(r"C:\Users\paulp\Downloads\Test", train=False) 
    ...
    resume_checkpoint = r"D:\SignDETR\checkpoints\190_model.pt"
    if os.path.exists(resume_checkpoint):
        model.load_state_dict(load(resume_checkpoint))
        start_epoch = 160
        logger.info(f"Resumed from checkpoint: {resume_checkpoint} (epoch {start_epoch})")
